...
int buscaBin(int *array,int begin, int end, int chave) {
	...
	if (begin == end) { //a recusividade faz com que, se o elemento nao for encontrado na parte do inicio ao meio(inferior) ou do meio ao fim (superior), o end seja iterado em -1 e begin seja em +1, assim encerrando e retornando -1
		...
		if (array[meio] < chave && array[meio + 1] == true) {
			return meio + 1;
		}
	...
	if (array[meio] == chave) { // se a chave se encontra no meio do array, ja se retorna a posi meio
		return meio;
	}

	if (array[meio] > chave) {
		//                           meio - 1 é passado como end (até que o meio seja a chave ou o novo end(meio - 1) seja menor que a posi begin)
