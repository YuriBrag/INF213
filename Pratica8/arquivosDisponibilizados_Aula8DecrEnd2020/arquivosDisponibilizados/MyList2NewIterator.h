...
template<class T>
class MyList2 { friend class MyList2Iterator<T>;
public:
...
public:
	MyList2Iterator(Node<T> *ptr_, MyList2<T> *ptrLista_): ptr(ptr_), ptrLista(ptrLista_) {}
...
MyList2Iterator<T> MyList2Iterator<T>::operator--(int) {
	MyList2Iterator<T> oldIt= *this;
	MyList2Iterator<T> itLast(ptrLista->dataLast,ptrLista);
	if(ptr == NULL){
...
template<class T>
int MyList2<T>::size() {/*etapa 2: implemente esta funcao sem usar o membro de dados dataSize*/ 
	int tam = 0;
	iterator curr = begin();
	if (curr == end()) { 
		return tam;
	...
	while(curr!= end()) { //enquanto nao for o ultimo
		tam++; //acrescenta um no tam
		curr++; //avance para o proximo nodo
	}
	return tam;
