...
public:
	...
	int sizeRecursivo(Node<T> *dataLast);
	int size() const {return sizeRecursivo(dataLast)}; 
...
template<class T>
void MyList2<class T>::reverse(){

}

template<class T>
int MyList2<class T>::eraseMatchingElements(const T& elem){
...
template<class T>
int MyList2<class T>::sizeRecursivo(Node<T> *p){
	if(p == NULL)
		return 0;
	else
		return 1 + sizeRecursivo(p->prev);
}
