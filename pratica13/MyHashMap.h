...
void MyHashMap<KEY, VALUE, HASH>::reHash(int newSize){
	...
	MyVec<MyList2<std::pair<KEY,VALUE> > > table_aux = table;
	table.clear();
	table.resize(newSize);

	for(int i=0;i<tableSize;i++) {
		for(typename MyList2<std::pair<KEY,VALUE> >::iterator it = table_aux[i].begin();it!=table_aux[i].end();it++) {
...
void MyHashMap<KEY,VALUE,HASH>::set(const  KEY &k, const VALUE &v) {
	...
	int pos = funcaoHash(k) % table.size();
	for(typename MyList2<std::pair<KEY,VALUE> >::iterator it = table[pos].begin();it!=table[pos].end();it++) { //procure pela chave no balde adequado da tabela...
		if((*it).first == k) { //encontramos a chave?
			(*it).second = v;
			return;
		}
	} 
	table[pos].push_back(make_pair(k, v));
	numElems++;
}
