...
//cria um node aux para guardar o proximo do dataFirst 
//apaga datafirst e seta o aux como novo datafirst e dimunui o tamanho de data

template<class T>
void MyList<T>::destroy(MyList<T> *first) { 
	//Termine esta funcao...
	if(empty()) {
		return;
	}
	MyList<T>::iterator aux = first->next;
	delete first;
	destroy(aux);
}
...
//para colocar um elemento na lista basta criar um novo nodo que é na verdade um iterator que aponta para o nodo dataFirst inicialmente, 
//depois disso basta fazer o dataFirst receber o aux
template<class T>
...
//insere um elemento no meio da lista, primeiro verifica se o que vem antes é valido, se nao esta fora da lista
//se for valido cria um iterador para esse novo elemento, faz o aux->next = nodeBefore->next  e  nodeBefore->next = aux (essa ordem importa, 
//pois se fizermos primeiro o aux ser o proximo do nodeBefore perdemos quem vem depois do nodeBefore)
//importante lembrar que como o newNode pega o lugar de nodeBefore, se nodeBefore antes era o dataLast o new node sera o novo dataLast
template<class T>
...
typename MyList<T>::iterator MyList<T>::erase(iterator elem) { //remove o elemento apontado por Elem													       
	dataSize--;                                                //retorna o (apontador) para o elemento apos o removido
	if(elem==dataFirst && elem==dataLast) { 				   //exercicio: por que precisamos desse caso especial? pois se so tiver um elemento na lista é necessario setar o datalast e dataFirst como null
...
int MyList<T>::eraseMatchingElements(const T &elem) {
	...
	while (curr != end()) {
		MyList<T>::iterator aux = curr->next;
		if (deref(curr) == elem) {
			cont++;
			erase(elem);
		}
