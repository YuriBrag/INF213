...
private:
	...
	void create();
	void destroy();
	// void destroy(MyList<T> *first);
};

template <class T>
void MyList<T>::destroy()
{
	MyList<T>::iterator aux;
...
{
	if (empty()){ //termina a funcao para quando acabar todos curr e na lista de operacoes bota o ultimo curr na primeira posi o penultimo na segunda e assim por diante
		return;
	}

	T curr = deref(dataFirst);
	erase(begin());
