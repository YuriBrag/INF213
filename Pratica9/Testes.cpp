...
void reverse(MyList2<T> &lista) {
  ...
  typename MyList2<int>::iterator currBeg = lista.begin();
  typename MyList2<int>::iterator currEnd = lista.begin();
  typename MyList2<int>::iterator currAux= lista.begin();

  while(currAux!= lista.end()){ //incrementa o currAux ate que chegue antes do null, fazendo gravar em curr end o penultimo valor
    currEnd = currAux;
		currAux++;
	}

  while (currBeg != currEnd) {
		swap(*currBeg, *currEnd);
		currBeg++;
    if(currBeg == currEnd)
      break;
    currEnd--;
