...
private:
	...
	iterator upper_bound(const K&key, MapNode<K,V> *root);
...
template <class K, class V>
typename MyMap<K,V>::iterator MyMap<K,V>::upper_bound(const K&key, MapNode<K,V> *root) { //funcao que retorna o iterador do primeiro maior que a chave passada caso ela nao esteja no map, se tiver mais de uma retorna a primeira
	if (!root){ //verifica se chegou ao fim do map (caso base)
		return iterator(NULL);
	}
	else{
		if (key<root->keyValue.first){
			MyMap<K, V>::iterator itLeft = upper_bound(key,root->left); //verifica para os menores(esquerda) caso a chave seja menor do que o curr
			if (itLeft == end()){ //verifica se chegou ao final dos filhos a esquerda
				return iterator(root);
			}
			else{ //retorna o it left para a ultima chamada recursiva 
				return itLeft;
			}
		}
		else if (root->keyValue.first < key){ //se a chave for maior que o curr, faz a pesquisa para a direita da arvore
			return upper_bound(key,root->right);
		}
		else{ //se nao for nem maior nem menor encontrou e retorna o curr
			return iterator(root);
		}
	}
}
...
template <class K, class V>
typename MyMap<K,V>::iterator MyMap<K,V>::upper_bound(const K&key) {
